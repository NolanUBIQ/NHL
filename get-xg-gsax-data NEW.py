...
if len(games) > 0:
    ...
    if "goalie-gsax-output.csv" not in os.listdir("./"):
        goalieGsax.to_csv("goalie-gsax-output.csv", index=False)
    else:
        prevGoalieGsax = pd.read_csv("goalie-gsax-output.csv")
        goalieGsax = pd.concat([prevGoalieGsax, goalieGsax], axis =0)
        goalieGsax = goalieGsax.drop_duplicates(subset=["gid", "Date", "playerName"])
        goalieGsax.to_csv("goalie-gsax-output.csv", index=False)

    if "xg-stats-output.csv" not in os.listdir("./"):
        xgStats.to_csv("xg-stats-output.csv", index=False)
    else:
        prevXgStats = pd.read_csv("xg-stats-output.csv")
